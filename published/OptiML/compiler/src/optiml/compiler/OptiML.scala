...
//    }
    ...
    // The main kernel's IR node is built with just 'fresh' nodes - so use the 
    // dependency list (deps) to generate initialization code
//    val argTypeList: ListBuffer[(String, String)] = getArgTypeFromSyms(deps) 
//    for (t: (String, String) <- argTypeList) {
//      if (!declaredSet.contains(t._1)) {
//        declaredSet += t._1
...
//    val argvs: Int = argsAndArgvs._2
    stream.println("  if (argc <= " + (argvs+1) + ") {")
